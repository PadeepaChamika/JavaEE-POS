...
public class CustomerDAOImpl implements CustomerDAO {
    ...
    public JsonArray getAll(Connection connection) throws SQLException, ClassNotFoundException {
        ...
        while (rst.next()) {
            Customer customer = new Customer(rst.getString(1), rst.getString(2),rst.getString(3), rst.getString(4), rst.getDouble(5));
            ...
            customerObj.add("age", customer.getAge());
    ...
    public boolean update(Connection connection, Customer customer) throws SQLException, ClassNotFoundException {
        return CrudUtil.executeUpdate(connection, "UPDATE Customer SET name=?,address=?,age=?,salary=? WHERE id=?", customer.getName(), customer.getAddress(), customer.getSalary(), customer.getId());
    }
